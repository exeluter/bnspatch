  ...
  for ( auto const &current : children ) {
    if ( !_wcsicmp(current.name(), xorstr_(L"parent")) ) {
      ...
      if ( !_wcsicmp(current.name(), xorstr_(L"set-name")) ) {
        ...
        process_patch_command_node({ context_node.insert_attribute_before(current.attribute(xorstr_(L"name")).value(), context_attr), context_node }, current.children());
      } else if ( !_wcsicmp(current.name(), xorstr_(L"insert-attribute-after")) ) {
        process_patch_command_node({ context_node.insert_attribute_after(current.attribute(xorstr_(L"name")).value(), context_attr), context_node }, current.children());
      ...
      if ( !_wcsicmp(current.name(), xorstr_(L"select-node")) ) {
        process_patch_command_node(context_node.select_node(current.attribute(xorstr_(L"query")).value()), current.children());
      } else if ( !_wcsicmp(current.name(), xorstr_(L"select-nodes")) ) {
        for ( auto &x : context_node.select_nodes(current.attribute(xorstr_(L"query")).value()) ) {
          process_patch_command_node(x, current.children());
        }
      } else if ( !_wcsicmp(current.name(), xorstr_(L"prepend-attribute")) ) {
        process_patch_command_node({ context_node.prepend_attribute(current.attribute(xorstr_(L"name")).value()), context_node }, current.children());
      } else if ( !_wcsicmp(current.name(), xorstr_(L"append-attribute")) ) {
        process_patch_command_node({ context_node.append_attribute(current.attribute(xorstr_(L"name")).value()), context_node }, current.children());
        ...
        auto x = context_node.select_node(current.attribute(xorstr_(L"query")).value());
        ...
        if ( auto node = context_node.select_node(current.attribute(xorstr_(L"query")).value()).node() ) {
        ...
        process_patch_command_node({ context_node.attribute(current.attribute(xorstr_(L"name")).value()), context_node }, current.children());
        ...
        process_patch_command_node(context_node.child(current.attribute(xorstr_(L"name")).value()), current.children());
        ...
        process_patch_command_node(context_node.find_child_by_attribute(current.attribute(xorstr_(L"name")).value(), current.attribute(xorstr_(L"attr-name")).value(), current.attribute(xorstr_(L"attr-value")).value()), current.children());
        ...
        process_patch_command_node(context_node.first_element_by_path(current.attribute(xorstr_(L"path")).value()), current.children());
      } else if ( !_wcsicmp(current.name(), xorstr_(L"insert-child-after")) ) {
        if ( auto node = context_node.select_node(current.attribute(xorstr_(L"query")).value()).node() )
          process_patch_command_node(context_node.insert_child_after(current.attribute(xorstr_(L"name")).value(), node), current.children());
      } else if ( !_wcsicmp(current.name(), xorstr_(L"insert-child-before")) ) {
        if ( auto node = context_node.select_node(current.attribute(xorstr_(L"query")).value()).node() )
          process_patch_command_node(context_node.insert_child_before(current.attribute(xorstr_(L"name")).value(), node), current.children());
      } else if ( !_wcsicmp(current.name(), xorstr_(L"insert-copy-after")) ) {
        // not implemented
      } else if ( !_wcsicmp(current.name(), xorstr_(L"insert-copy-before")) ) {
        // not implemented
  ...
  class XmlPieceReader *arg4)
{
  ...
  if ( fileName && xmlDoc && xmlDoc->IsValid() ) {
    ...
    if ( !vec.empty() ) {
      ...
      if ( !_wcsicmp(fs::path(fileName).extension().c_str(), xorstr_(L".x16")) ) {
