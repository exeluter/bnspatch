...
class pattern_searcher
{
  ...
  std::chrono::duration<Rep, Period> *_elapsed_duration;

public:
  pattern_searcher(const char *pattern, std::chrono::duration<Rep, Period> *elapsed_duration = nullptr)
  {
    _cp = compile_pattern(pattern);
    _clock = std::chrono::high_resolution_clock();
    _elapsed_duration = elapsed_duration;
  }
  ...
  std::pair<Iterator, Iterator> operator()(Iterator first, Iterator last) const
  {
    ...
    auto then = _clock.now();
    auto it = std::search(first, last, std::begin(*_cp), std::end(*_cp));
    auto now = _clock.now();
    if ( _elapsed_duration )
      *_elapsed_duration = now - then;
